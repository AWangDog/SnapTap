...
import sys, os, configparser, ctypes, pynput, WinKeyBoard, subprocess, signal, tempfile, psutil, multiprocessing
...
def click_left_mouse_button(event, f13_pressed):
    while True:
        event.wait()
        while f13_pressed.value:
            user32.mouse_event(MOUSEEVENTF_LEFTDOWN, 0, 0, 0, 0)
            user32.mouse_event(MOUSEEVENTF_LEFTUP, 0, 0, 0, 0)

def click_right_mouse_button(event, f14_pressed):
    while True:
        event.wait()
        while f14_pressed.value:
            user32.mouse_event(MOUSEEVENTF_RIGHTDOWN, 0, 0, 0, 0)
            user32.mouse_event(MOUSEEVENTF_RIGHTUP, 0, 0, 0, 0)

def click_left():
    f13_pressed = multiprocessing.Value('i', 0)  # 使用共享变量
    event = multiprocessing.Event()
    
    process = multiprocessing.Process(target=click_left_mouse_button, args=(event, f13_pressed))
    process.start()

    def on_press(key):
        if key == pynput.keyboard.Key.f13 and not f13_pressed.value:
            f13_pressed.value = 1
            event.set()
        elif key == pynput.keyboard.Key.f13 and f13_pressed.value:
            f13_pressed.value = 0
            event.clear()

    def on_release(key):
        if key == pynput.keyboard.Key.f13:
            f13_pressed.value = 0
            event.clear()
...
def click_right():
    f14_pressed = multiprocessing.Value('i', 0)  # 使用共享变量
    event = multiprocessing.Event()

    process = multiprocessing.Process(target=click_right_mouse_button, args=(event, f14_pressed))
    process.start()

    def on_press(key):
        if key == pynput.keyboard.Key.f14 and not f14_pressed.value:
            f14_pressed.value = 1
            event.set()
        elif key == pynput.keyboard.Key.f14 and f14_pressed.value:
            f14_pressed.value = 0
            event.clear()

    def on_release(key):
        if key == pynput.keyboard.Key.f14:
            f14_pressed.value = 0
            event.clear()
...
if __name__ == '__main__':
    # 启动程序
    left_process = multiprocessing.Process(target=click_left)
    right_process = multiprocessing.Process(target=click_right)
    left_process.start()
    right_process.start()
